      ...
      <div className="p-4 space-y-4">
        <div className="flex space-x-2">
          {categories.map((category, index) => (
            <span key={index} className="bg-red-600 text-white text-xs font-bold px-2 py-1 rounded">{category}</span>
          ))}
        </div>
        <h1 className="font-bold text-2xl text-gray-800">{title}</h1>
        <p className="text-gray-600">{content}</p>
        <div className="flex items-center space-x-2">
          <img src="/images/avatar.png" className="w-8 h-8 object-cover rounded-full" />
          <div>
